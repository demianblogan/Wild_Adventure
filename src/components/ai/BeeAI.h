...
namespace ECS
{
	...
	struct BeeAI
	{
		...
		bool  isInitialized = false; // anchor is captured on the first update
		float anchorX = 0.0f;
		float anchorY = 0.0f;
		...
		bool  hasFiredBullet = false; // true once the bullet is spawned for the current cycle

		static constexpr float ShootInterval = 0.25f;     // seconds between shots
		static constexpr float AttackRangeFactor = 3.0f; // x the bee's collider width
		static constexpr float BulletSpeed = 220.0f;      // downward bullet speed
		static constexpr int   FireFrame = 4;             // 5th attack frame (0-based)
	};
}
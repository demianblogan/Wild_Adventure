...
// kicks the screen (via `shake`) and scatters a burst of pixel dust.
// `container` must already be positioned/sized by its parent (anchor/pivot/
// offset) -- this only fills in its size and children. onAllLanded fires
// once the last letter has finished settling.
